...
select count(p.id_departamento) as cantidad_profes , d.nombre from profesor p inner join departamento d on p.id_departamento = d.id group by 2 order by 1 desc;
...
-- departamentos que no tienen profesores asociados. Estos departamentos también tienen que aparecer en el listado.
 select * from departamento;
select d.nombre, count(p.id_departamento) as cantidad_profes from departamento d left join profesor p on d.id = p.id_departamento group by 1;
...
select * from asignatura;
select grado.nombre, count(asignatura.id) as cantidad_asignatura from grado inner join asignatura on grado.asignatura_id = asignatura.grado_id group by asignatura.asignatura_id having count(asignatura.id) > 40;
